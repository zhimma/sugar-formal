...
//                    $user_b->notify(new AccountConsign('交付帳號開啟通知',$user_b->name, $content_b));
    ...
    public function viewVipForPaid(Request $request)
    {
        ...
        return view('new.dashboard.vipForPaid')
            ->with('user', $user)->with('cur', $user)
            ->with('vip_text', $vip_text->content)
            ->with('upgrade_vip', $upgrade_vip->content)
            ->with('cancel_vip', $cancel_vip->content)
...
//        return redirect('/dashboard/evaluation/'.$request->input('eid'))->with('message', '評價回覆已完成');
    ...
    public function search2(Request $request)
    {
        ...
        $search_tag_array = $request->search_tag;
        if($search_tag_array)
        {
                ...
                if(!in_array($type->type_name, $exclude_type_array))
                {
                    if(Schema::hasColumn($table_name, 'is_custom')) {
                        $temp_option_list = DB::table($table_name)
                                                ->where('is_custom', 1)
                                                ->whereIn('option_name', $search_tag_array)
                                                ->get();
            ...
            if(count($option_list))
            {
                ...
                $search_tag_array = $temp_search_tag_array;
            }
            $search_tag_array = array_unique($search_tag_array);
            request()->search_tag = $search_tag_array;
        }
