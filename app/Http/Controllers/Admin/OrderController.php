    ...
    public function orderEcPayCheck(Request $request){
        ...
        if($paymentData['TradeStatus']==1){
            ...
            if(isset($paymentPeriodInfo) && $paymentPeriodInfo['ExecLog'] != ''){
                ...
                if($currentOrder->pay_fail != 'null' && $currentOrder->pay_fail != null && $currentOrder->pay_fail != ''){
...
//                dd($currentOrder->pay_fail);
                if($currentOrder &&
                    ((Carbon::parse($lastProcessDate)->toDateTimeString() != Carbon::parse($current_order_pay_date[0])->toDateTimeString() &&
                            (($currentOrder->pay_fail != 'null' && $currentOrder->pay_fail != '' && Carbon::parse($lastProcessDate)->toDateTimeString() != Carbon::parse($current_order_pay_fail[0])->toDateTimeString()) || ($currentOrder->pay_fail=='' || $currentOrder->pay_fail == null || $currentOrder->pay_fail == 'null'))
                    ) ||
    ...
    public function orderCheckByServiceNameOrOrderId(Request $request){
        ...
        if($order && count($order)>0){
            ...
            foreach ($order as $row){
                ...
                if($row->pay_fail != '' && $row->pay_fail != "null" && json_decode($row->pay_fail) != null) {
