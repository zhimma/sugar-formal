...
class AdjustedPeriodCheck
{
    ...
    public function handle(Request $request, Closure $next)
    {
        ...
        if($user->engroup == 2)
        {
            ...
            {
                if($user_login_count >= 3 && $user_login_count <= 10 && $user_provisional_variables->has_adjusted_period_first_time == 0 && (!$request->session()->has('first_exchange_period_modify_next_time')))
                {
