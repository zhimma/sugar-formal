...
class fileUploader_helper {
	 ...
	 * @return {Array}
     */
    public function upload($edit = null, $hasChangedFileName = null) {
    	return $this->uploadFiles($edit, $hasChangedFileName);
    }
    ...
    private function validate($item = null) {
        if ($item == null) {
            ...
            $ini = array(
				...
				str_ends_with(ini_get('upload_max_filesize'), 'G') ? ((int)ini_get('upload_max_filesize')) * 1024: ((int)ini_get('upload_max_filesize')),
                str_ends_with(ini_get('post_max_size'), 'G') ? ((int)ini_get('post_max_size')) * 1024: ((int)ini_get('post_max_size')),
    ...
    private function checkExtension($filename, $oldname)
    {
        $extension = pathinfo($filename);
        $oldNameExtension = pathinfo($oldname);

        if (isset($extension['extension'])) {
            return $filename;
        } else {
            return $extension['filename'] . '.' . $oldNameExtension['extension'];
        }
	}
     ...
     * @return {null, Array}
     */
	private function uploadFiles($edit = null, $hasChangedFileName = null) {
		$data = array(
			"hasWarnings" => false,
			"isSuccess" => false,
			"warnings" => array(),
		...
		if ($this->field['input']) {
			...
			if ($validate === true) {
                // process the files
				$count = count($this->field['listInput']['list']);
				$j = 0;
				for($i = 0; $i < $count; $i++) {
				    if (isset($this->field['listInput']['values'][$i]['is_exists_file']) && $this->field['listInput']['values'][$i]['is_exists_file'] === 1) {
				        continue;
                    }

					$file = array(
						'name' => $hasChangedFileName && isset($this->field['listInput']['values'][$i]['name']) ?
                            $this->checkExtension($this->field['listInput']['values'][$i]['name'], $this->field['input']['name'][$j]):
                            $this->field['input']['name'][$j],
						'old_name' => $this->field['input']['name'][$j],
						'tmp_name' => $this->field['input']['tmp_name'][$j],
						'type' => $this->field['input']['type'][$j],
						'error' => $this->field['input']['error'][$j],
						'size' => $this->field['input']['size'][$j]
					);
					...
					$metas['old_name'] = $file['old_name'];
					$metas['title'] = $hasChangedFileName ? $file['name']: substr($metas['old_name'], 0, (strlen($metas['extension']) > 0 ? -(strlen($metas['extension'])+1) : strlen($metas['old_name'])));
					...
					$listInputName = $hasChangedFileName ? pathinfo($metas['old_name'])['filename']: '0:/' . $metas['old_name'];
					$fileInList = $listInput === null || in_array($hasChangedFileName && isset($this->field['listInput']['values'][$i]['name']) ? pathinfo($metas['name'])['filename'] :$listInputName,
                            $hasChangedFileName && isset($this->field['listInput']['values'][$i]['name']) ?
                                array_map(function ($item) {
                                return pathinfo($item)['filename'];
                            }, Arr::pluck($listInput['values'], 'name')):
                                array_map(function ($item) {
                                return pathinfo($item)['filename'];
                            }, $listInput['list'])
                        );
					...
					if ($validateFile === true) {
						if ($fileInList) {
							if ($listInput) {
                                $metas['listProps'] = $listInput['values'][$i];
                                unset($listInput['list'][$i]);
                                unset($listInput['values'][$i]);
                            }

							$tempName = $this->generateFileName($this->options['title'], array_diff_key($metas, array_flip(array('tmp_name', 'error', 'chunked'))));

                            $metas['i'] = count($data['files']);
							$metas['name'] = $hasChangedFileName ? pathinfo($tempName)['filename'] :$this->generateFileName($this->options['title'], array_diff_key($metas, array_flip(array('tmp_name', 'error', 'chunked'))));
							$metas['title'] = substr($metas['name'], 0, (strlen($metas['extension']) > 0 ? -(strlen($metas['extension'])+1) : strlen($metas['name'])));
							$metas['file'] = $uploadDir . $tempName;
        ...
        if ($listInput) {
            ...
            if ($edit) {
                foreach ($listInput['list'] as $key => $item) {
                    if (substr($item, 0, 3) !== '0:/') {
                        $item = ltrim($item, '/');
                        $type = self::mime_content_type($item);
                        $oldInput = [];
                        $oldInput['date'] = date('r');
                        $oldInput['extension'] = strtolower(substr(strrchr($item, "."), 1));
                        $oldInput['file'] = $item;
                        $oldInput['format'] = strtok($type, '/');
                        $oldInput['listProps'] = $listInput['values'][$key];
                        $oldInput['name'] = substr(strrchr($item, "/"), 1);
                        $oldInput['old_name'] = $oldInput['name'];
                        $oldInput['old_title'] = substr($oldInput['old_name'], 0, (strlen($oldInput['extension']) > 0 ? -(strlen($oldInput['extension'])+1) : strlen($oldInput['old_name'])));
                        $oldInput['replaced'] = false;
                        $oldInput['size'] = filesize($item);
                        $oldInput['size2'] = $this->formatSize($oldInput['size']);
                        $oldInput['title'] = $oldInput['old_title'];
                        $oldInput['type'] = $type;
                        $oldInput['uploaded'] = true;

                        if (isset($oldInput['listProps']['file'])) {
                            $oldInput['listProps']['file'] = $item;
                        }

                        $this->options['files'][] = $oldInput;
                    }
                }
            }
        }
    ...
    private function codeToMessage($code, $file = null) {
	    ...
	    switch ($code) {
            ...
            case 'accepted_file_types':
                $message = $file['old_name']."格式不符合，只允許上傳圖片格式(JPG、JPEG、PNG)" ;
                break;
            case 'file_uploads':
                $message = "服務器不允許上傳圖片";
                break;
            case 'max_file_size':
                $message = $file['old_name'] . "，圖片文件過大";
                break;
            case 'max_files_size':
                $message = "檔案過大";
                break;
