...
class Banned
{
    ...
    public static function addRemindMsgFromBannedId($banned_user_id) {
        $relateUserIds = [];
        $admin_id = AdminService::checkAdmin()->id;
        $banned_user = User::findById($banned_user_id);
        $msgOfBannedUser = Message_new::allSenders($banned_user->id,$banned_user->isVip(),'curMon',false);
        foreach($msgOfBannedUser  as $msg) {
            if($msg->to_id==$banned_user_id && !in_array($msg->from_id,$relateUserIds)) $relateUserIds[] = $msg->from_id;
            if($msg->from_id==$banned_user_id && !in_array($msg->to_id,$relateUserIds)) $relateUserIds[] = $msg->to_id;
        }
