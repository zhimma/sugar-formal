...
! function (a) {
    ...
    f.prototype = {
        ...
        _resetOptions: function (a) {
            this.domOptions = d.objectCreate(this.parent.options), this.options = d.objectCreate(this.domOptions);
        ...
        trigger: function (a, b) {
            b = b || this;
            var d, c = this._listeners && this._listeners[a];
            if (c)
                for (var f = c.length; f--;)
                    if (d = c[f].call(b, b), d === !1) return d;
    ...
    var g = function () {
        ...
        e.prototype = {
            ...
            hasGroup: function (a) {
                return g(a) ? this.hasOneOf(a) : "Any" === a || (this.hasGroups() ? -1 !== this.groups.indexOf(a) : "Default" === a)
            },
            hasOneOf: function (a) {
            ...
            Collection: function (a) {
                return this.__class__ = "Collection", this.constraint = "undefined" != typeof a && (a instanceof e ? a : new c(a)), this.validate = function (a, c) {
                    var e, h = new b,
                        i = 0,
                        j = {},
    ...
    g = "undefined" != typeof g ? g : "undefined" != typeof module ? module.exports : null;
    var h = function (a, b) {
        this.__class__ = "ParsleyValidator", this.Validator = g, this.locale = "en", this.init(a || {}, b || {})
    };
    h.prototype = {
        init: function (b, c) {
        ...
        validate: function (a, b, c) {
            return (new this.Validator.Validator).validate.apply(new g.Validator, arguments)
        },
        addValidator: function (a, b, c, f) {
            if (this.validators[a]) d.warn('Validator "' + a + '" is already defined.');
            else if (e.hasOwnProperty(a)) return void d.warn('"' + a + '" is a restricted keyword and is not a valid validator name.');
            return this._setValidator(a, b, c, f)
        },
        updateValidator: function (a, b, c, e) {
            return this.validators[a] ? this._setValidator(a, b, c, e) : (d.warn('Validator "' + a + '" is not already defined.'), this.addValidator(a, b, c, e))
        },
        removeValidator: function (a) {
            return this.validators[a] || d.warn('Validator "' + a + '" is not defined.'), delete this.validators[a], this
        },
        _setValidator: function (b, c, d, e) {
            return this.validators[b] = function (b) {
                return a.extend((new g.Assert).Callback(c, b), {
                    priority: d,
        ...
        validators: {
            notblank: function () {
                return a.extend((new g.Assert).NotBlank(), {
                    priority: 2
                })
            },
            required: function () {
                return a.extend((new g.Assert).Required(), {
                    priority: 512
                })
            },
            type: function (b) {
                ...
                switch (b) {
                    case "email":
                        c = (new g.Assert).Email();
                        break;
                    case "range":
                    case "number":
                        c = (new g.Assert).Regexp("^-?(?:\\d+|\\d{1,3}(?:,\\d{3})+)?(?:\\.\\d+)?$");
                        break;
                    case "integer":
                        c = (new g.Assert).Regexp("^-?\\d+$");
                        break;
                    case "digits":
                        c = (new g.Assert).Regexp("^\\d+$");
                        break;
                    case "alphanum":
                        c = (new g.Assert).Regexp("^\\w+$", "i");
                        break;
                    case "url":
                        c = (new g.Assert).Regexp("^(?:(?:https?|ftp)://)?(?:\\S+(?::\\S*)?@)?(?:(?:[1-9]\\d?|1\\d\\d|2[01]\\d|22[0-3])(?:\\.(?:1?\\d{1,2}|2[0-4]\\d|25[0-5])){2}(?:\\.(?:[1-9]\\d?|1\\d\\d|2[0-4]\\d|25[0-4]))|(?:(?:[a-z\\u00a1-\\uffff0-9]-*)*[a-z\\u00a1-\\uffff0-9]+)(?:\\.(?:[a-z\\u00a1-\\uffff0-9]-*)*[a-z\\u00a1-\\uffff0-9]+)*(?:\\.(?:[a-z\\u00a1-\\uffff]{2,})))(?::\\d{2,5})?(?:/\\S*)?$", "i");
                        break;
                    default:
            ...
            pattern: function (b) {
                var c = "";
                return /^\/.*\/(?:[gimy]*)$/.test(b) && (c = b.replace(/.*\/([gimy]*)$/, "$1"), b = b.replace(new RegExp("^/(.*?)/" + c + "$"), "$1")), a.extend((new g.Assert).Regexp(b, c), {
                    priority: 64
                })
            },
            minlength: function (b) {
                return a.extend((new g.Assert).Length({
                    min: b
                }), {
                    priority: 30,
            ...
            min: function (b) {
                return a.extend((new g.Assert).GreaterThanOrEqual(b), {
                    priority: 30,
                    requirementsTransformer: function () {
                        return "string" != typeof b || isNaN(b) ? b : parseInt(b, 10)
                    }
            ...
            max: function (b) {
                return a.extend((new g.Assert).LessThanOrEqual(b), {
                    priority: 30,
                    requirementsTransformer: function () {
                        return "string" != typeof b || isNaN(b) ? b : parseInt(b, 10)
                    }
            ...
            range: function (b) {
                return a.extend((new g.Assert).Range(b[0], b[1]), {
                    priority: 32,
                    requirementsTransformer: function () {
            ...
            equalto: function (b) {
                return a.extend((new g.Assert).EqualTo(b), {
                    priority: 256,
                    requirementsTransformer: function () {
    ...
    i.prototype = {
        ...
        _insertErrorWrapper: function (b) {
            ...
            if ("string" == typeof b.options.errorsContainer) {
                ...
                d.warn("The errors container `" + b.options.errorsContainer + "` does not exist in DOM")
            ...
            var e = b.$element;
            return b.options.multiple && (e = e.parent()), e.after(b._ui.$errorsWrapper)
        },
    ...
    var j = function (b, c, e) {
        this.__class__ = "ParsleyForm", this.__id__ = d.generateID(), this.$element = a(b), this.domOptions = c, this.options = e, this.parent = window.Parsley, this.fields = [], this.validationResult = null
    };
    j.prototype = {
    ...
    var k = function (b, c, e, f, g) {
            var h = {};
            if (!new RegExp("ParsleyField").test(b.__class__)) throw new Error("ParsleyField or ParsleyFieldMultiple instance expected");
            if ("function" == typeof window.ParsleyValidator.validators[c] && (h = window.ParsleyValidator.validators[c](e)), "Assert" !== h.__parentClass__) throw new Error("Valid validator expected");
            var i = function () {
                return "undefined" != typeof b.options[c + "Priority"] ? b.options[c + "Priority"] : h.priority || 2
            };
            return f = f || i(), "function" == typeof h.requirementsTransformer && (e = h.requirementsTransformer(), h = window.ParsleyValidator.validators[c](e)), a.extend(h, {
                name: c,
                requirements: e,
                priority: f,
                groups: [f],
                isDomConstraint: g || d.checkAttr(b.$element, b.options.namespace, c)
            })
        },
        l = function (b, c, e, f) {
            this.__class__ = "ParsleyField", this.__id__ = d.generateID(), this.$element = a(b), "undefined" != typeof f && (this.parent = f), this.options = e, this.domOptions = c, this.constraints = [], this.constraintsByName = {}, this.validationResult = [],
    ...
    l.prototype = {
        ...
        _handleWhitespace: function (a) {
            return !0 === this.options.trimValue && d.warnOnce('data-parsley-trim-value="true" is deprecated, please use data-parsley-whitespace="trim"'), "squish" === this.options.whitespace && (a = a.replace(/\s{2,}/g, " ")), "trim" !== this.options.whitespace && "squish" !== this.options.whitespace && !0 !== this.options.trimValue || (a = a.replace(/^\s+|\s+$/g, "")), a
        },
    ...
    n.prototype = {
        init: function (a) {
            return this.__class__ = "Parsley", this.__version__ = "2.1.3", this.__id__ = d.generateID(), this._resetOptions(a), this.$element.is("form") || d.checkAttr(this.$element, this.options.namespace, "validate") && !this.$element.is(this.options.inputs) ? this.bind("parsleyForm") : this.isMultiple() ? this.handleMultiple() : this.bind("parsleyField")
        },
        isMultiple: function () {
        ...
        handleMultiple: function () {
            ...
            this.options.multiple = this.options.multiple.replace(/(:|\.|\[|\]|\{|\}|\$)/g, ""), "undefined" != typeof c && a('input[name="' + c + '"]').each(function () {
                a(this).is("input[type=radio], input[type=checkbox]") && a(this).attr(b.options.namespace + "multiple", b.options.multiple)
            });
            for (var g = this._findRelatedMultiple(), h = 0; h < g.length; h++)
                if (f = a(g.get(h)).data("Parsley"), "undefined" != typeof f) {
                    this.$element.data("ParsleyFieldMultiple") || f.addElement(this.$element);
                    break
                }
            return this.bind("parsleyField", !0), f || this.bind("parsleyFieldMultiple")
        },
        bind: function (b, c) {
            var e;
            switch (b) {
                case "parsleyForm":
                    e = a.extend(new j(this.$element, this.domOptions, this.options), window.ParsleyExtend)._bindFields();
                    break;
                case "parsleyField":
                    e = a.extend(new l(this.$element, this.domOptions, this.options, this.parent), window.ParsleyExtend);
                    break;
                case "parsleyFieldMultiple":
                    e = a.extend(new l(this.$element, this.domOptions, this.options, this.parent), new m, window.ParsleyExtend)._init();
                    break;
                default:
                    throw new Error(b + "is not a supported Parsley type")
            }
            return this.options.multiple && d.setAttr(this.$element, this.options.namespace, "multiple", this.options.multiple), "undefined" != typeof c ? (this.$element.data("ParsleyFieldMultiple", e), e) : (this.$element.data("Parsley", e), e._trigger("init"), e)
        }
    };
    var o = a({}),
        p = function () {
    ...
    a.listen = function (a, b) {
        var c;
        if (p(), "object" == typeof arguments[1] && "function" == typeof arguments[2] && (c = arguments[1], b = arguments[2]), "function" != typeof arguments[1]) throw new Error("Wrong parameters");
        window.Parsley.on(s(a), q(b, c))
    }, a.listenTo = function (a, b, c) {
        if (p(), !(a instanceof l || a instanceof j)) throw new Error("Must give Parsley instance");
        if ("string" != typeof b || "function" != typeof c) throw new Error("Wrong parameters");
        a.on(s(b), q(c))
    }, a.unsubscribe = function (a, b) {
        if (p(), "string" != typeof a || "function" != typeof b) throw new Error("Wrong arguments");
        window.Parsley.off(s(a), b.parsleyAdaptedCallback)
    }, a.unsubscribeTo = function (a, b) {
        if (p(), !(a instanceof l || a instanceof j)) throw new Error("Must give Parsley instance");
        a.off(s(b))
    }, a.unsubscribeAll = function (b) {
        p(), window.Parsley.off(s(b)), a("form,input,textarea,select").each(function () {
            var c = a(this).data("Parsley");
            c && c.off(s(b))
        })
    }, a.emit = function (a, b) {
        p();
        var c = b instanceof l || b instanceof j,
            d = Array.prototype.slice.call(arguments, c ? 2 : 1);
        d.unshift(s(a)), c || (b = window.Parsley), b.trigger.apply(b, d)
        ...
        length: "輸入數字請介於 %s ~ %s 之間.",
        mincheck: "此區塊組合你必須要勾選%s項計畫或一項三次.",
    ...
    return a.extend(l.prototype, f.prototype), a.extend(j.prototype, f.prototype), a.extend(n.prototype, f.prototype), a.fn.parsley = a.fn.psly = function (b) {
        if (this.length > 1) {
            var c = [];
        ...
        return a(this).length ? new n(this, b) : void d.warn("You must bind Parsley on an existing element.")
    }, "undefined" == typeof window.ParsleyExtend && (window.ParsleyExtend = {}), t.options = a.extend(d.objectCreate(e), window.ParsleyConfig), window.ParsleyConfig = t.options, window.Parsley = window.psly = t, window.ParsleyUtils = d, window.ParsleyValidator = new h(window.ParsleyConfig.validators, window.ParsleyConfig.i18n), window.ParsleyUI = "function" == typeof window.ParsleyConfig.ParsleyUI ? (new window.ParsleyConfig.ParsleyUI).listen() : (new i).listen(), !1 !== window.ParsleyConfig.autoBind && a(function () {
