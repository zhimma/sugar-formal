            ...
            <span style="text-align:right;" class="m-portlet__head-text">
                ...
                @else
                    <form method="post" action="{{ route('deleteAll', ['uid' => $user->id]) }}">
                        ...
                        <input type="submit" class="btn btn-danger m-btn m-btn--air m-btn--custom" value="刪除全部" />
                    </form>
                @endif
            </span>
                ...
                @endif
                        ...
                        <div class="m-widget3__delete">
                            <form method="post" action="{{ route('deleteSingle', ['uid' => $user->id, 'sid' => $to->id, 'ct_time' => $message->created_at, 'content' => $message->content]) }}">
                                <input type="hidden" name="_token" value="{{ csrf_token() }}">
                                <input type="submit" class="btn btn-danger m-btn m-btn--air m-btn--custom" value="刪除" />
                                <a href='{!! route('reportMessage', ['id' => $message->id, 'sid' => $to->id]) !!}' class="btn btn-danger m-btn m-btn--air m-btn--custom">檢舉</a>
                            </form>
                        </div>
                    </div>
                </div>
