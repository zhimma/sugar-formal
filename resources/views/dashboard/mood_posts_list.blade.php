		...
		<div class="row">
			...
			<div class="col-sm-12 col-xs-12 col-md-10">
				<div class="shou" style="text-align: center; position: relative;">
					<a href="/mood/posts_list" class="toug_back btn_img" style=" position: absolute; left: 0;">
							...
							@foreach($posts as $post)
								<li {{ $post->uid==1049 && $post->top==0 ? 'style=background:#ddf3ff;padding:10px' : ''}} @if($post->top==1) style="background:#ffcf869e;padding:10px;" @endif @if($post->deleted_by != null || $post->deleted_at != null) class="huis_02" @endif>
									<div class="tou_tx">
										...
										<a @if($post->deleted_at && $user->id != 1049) onclick="delete_alert()" @else href="/mood/post_detail/{{$post->pid}}" @endif>
											<font><i class="ne_talicon"><img src="/posts/images/tl_icon.png">{{ \App\Models\PostsMood::where('reply_id',$post->pid)->get()->count() }}</i></font>
										</a>
									</div>
									<a @if($post->deleted_at && $user->id != 1049) onclick="delete_alert()" @else href="/mood/post_detail/{{$post->pid}}" @endif>
										<div class="tc_text_aa"><span>{{$post->ptitle}}</span></div>
										<div class="tc_text_bb" style="word-break: break-all;"><p>{!! \App\Models\PostsMood::showContent($post->pcontents) !!}</p></div>
									</a>
								</li>
							@endforeach
						</ul>
	...
	<script>
		...
		function checkUserVip() {
			...
			if(checkUserVip==0) {
				...
				window.location.href = "/mood/posts";
			}
		}
	</script>
@stop