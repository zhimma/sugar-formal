<div wire:poll>
    ...
    @endphp
    @foreach($forumChatContent as $row)
        <div class="@if($row->user_id == auth()->user()->id) show @else send @endif maspp0">
            ...
            <div class="msg @if($row->user_id == auth()->user()->id) msg1 @endif">
                ...
                @else
                    <a class="chatWith" href="{{ url('/dashboard/viewuser/' . $row->user_id ) }}">
                ...
                <div class="@if($row->user_id == auth()->user()->id) msg_p1 @else msg_p @endif" style="color: {{$row->color}};">
                    @if($row->user_id != auth()->user()->id)<span class="nickname">{{$row->name}}</span>@endif
{{--                        <span class="nickname">{{$row->name}}</span>--}}
                    <i class="@if($row->user_id == auth()->user()->id) msg_input_nn_2 @else msg_input_nn @endif"></i>
                    {{$row->content}}
                            ...
                            <div class="row">
                                @foreach(json_decode($row->pic,true) as $key => $pic)
                                    @if(isset($pic['file_path']))
                                        <div class="col-xs-6 col-sm-6 col-md-4 col-lg-3" @if($row->user_id == auth()->user()->id) style="float: right;" @endif>
                    ...
                    @endif
                        <font class="sent_ri @if($row->user_id == auth()->user()->id) dr_l @else dr_r @endif"><span>{{substr($row->created_at,11,5)}}</span></font>
                </div>
            </div>
        </div>
        @php
    ...
    {{ $forumChatContent->links('livewire::sg-pages2') }}
</div>
