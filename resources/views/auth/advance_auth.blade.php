								...
								<form id="advance_auth_form" name="advance_auth_form" class="m-form m-form--fit" method="POST" action="/advance_auth_process">
									...
									<div class="de_input mtb20">
										...
										<div class="zybg_new02 @if(in_array('p',Session::get('error_code')??[])) has_error @endif">
											...
											<input name="phone_number" id="phone_number" type="text" class="xy_input xy_left" placeholder="請輸入手機號碼"  autocomplete="off">
										</div>
...
<script>
    ...
    function tab_agree() {
        ...
        clear_error_appear();
        if(check_val()) {
            $("#tab_confirm").show().find('.bltext').html('本站會將您的門號以及生日同步更新到會員基本資料，<span class="bolder red">身分證字號則只用在本次驗證並不會紀錄</span>')
