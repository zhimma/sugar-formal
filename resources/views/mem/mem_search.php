<!DOCTYPE html>
<html lang="en">

  <head>
    <meta charset="utf-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="format-detection" content="telephone=no" />
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <!-- The above 3 meta tags *must* come first in the head; any other head content must come *after* these tags -->
    <title>男生搜索</title>
    <!-- Bootstrap -->
    <link href="/css/mem/bootstrap.min.css" rel="stylesheet">
    <link href="/css/mem/bootstrap-theme.min.css" rel="stylesheet">
    <!-- owl-carousel-->
    <!--    css-->
    <link rel="stylesheet" href="/css/mem/style.css">
    <link rel="stylesheet" href="/css/mem/swiper.min.css">
    <script src="/js/mem/bootstrap.min.js"></script>
    <script src="/js/mem/jquery-2.1.1.min.js" type="text/javascript"></script>
    <script src="/js/mem/main.js" type="text/javascript"></script>

  </head>

  <body>
    <div class="head hetop">
      <div class="container">
        <div class="col-sm-12 col-xs-12 col-md-12"><img src="/img/mem/icon_41.png" class="logo" />
        </div>
      </div>
    </div>
    <div class="head heicon">
      <div class="container">
        <div class="col-sm-12 col-xs-12 col-md-12">
          <div class="commonMenu">
            <div class="menuTop">
              <img src="/img/mem/icon_41.png" class="logo" />
              <span id="menuButton"><img src="/img/mem/icon.png" class="he_img"></span>
            </div>
            <ul id="menuList" class="change marg30">
                            <div class="comt"><img src="/img/mem/t.png"></div>
                            <div class="coheight">
              <div class="heyctop">測試系統賬號</div>
              <div class="helist">
                <ul>
                  <li><a href=""><img src="/img/mem/icon_38.png">搜索</a></li>
                  <li><a href=""><img src="/img/mem/icon_45.png">訊息</a><span>10</span></li>
                  <li><a href=""><img src="/img/mem/icon_46.png">名單</a></li>
                  <li><a href=""><img src="/img/mem/icon_48.png">我的</a></li>
                </ul>
              </div>
              <a href="" class="tcbut">LOGOUT</a>
                            </div>
            </ul>
          </div>
        </div>
      </div>
    </div>

    <!---->
    <div class="container matop70">
      <div class="row">
        <div class="col-sm-2 col-xs-2 col-md-2 dinone">
          <div class="leftbg">
            <div class="leftimg"><img src="/img/mem/icon_03.png">
              <h2>測試系統賬號</h2></div>
            <div class="leul">
              <ul>
                  <li><a href=""><img src="/img/mem/icon_38.png">搜索</a></li>
                  <li><a href=""><img src="/img/mem/icon_45.png">訊息</a><span>10</span></li>
                  <li><a href=""><img src="/img/mem/icon_46.png">名單</a></li>
                  <li><a href=""><img src="/img/mem/icon_48.png">我的</a></li>
                    <li><a href=""><img src="/img/mem/iconout.png">退出</a></li>
              </ul>
            </div>
          </div>
        </div>
        <div class="col-sm-12 col-xs-12 col-md-10">
          <div class="shou shou02 sh_line"><span>搜索</span>
            <font>search</font>
          </div>
                    
                    <div class="n_search">
                   <div class="n_input">
                                <dt>
                                                        <span>區市</span>
                                                        <span>
                                                        <select name="" class="select_xx06"><option>連江縣</option><option>B</option></select>
                                                        <select name="" class="select_xx06 right"><option>南竿鄉</option><option>B</option></select>
                                                        </span>
                                                    </dt>
                                   <dt>
                                                        <span>年齡範圍</span>
                                                        <span>
                                                        <select name="" class="select_xx06 left"><option>連江縣</option><option>B</option></select><div class="sew6">至</div>
                                                        <select name="" class="select_xx06  right"><option>南竿鄉</option><option>B</option></select>
                                                        </span>
                                                    </dt>
                                                     <dt>
                                                         <div class="n_se left">
                                                             <span>預算</span>
                                                           <select name="" class="select_xx01"><option>100—200萬</option><option>300—400萬</option></select>
                                                         </div>
                                                         <div class="n_se right">
                                                             <span>抽菸</span>
                                                           <select name="" class="select_xx01"><option>是</option><option>否</option></select>
                                                         </div>
                                </dt>
                                <dt>
                                                        <span>體型</span>
                                                        <span class="line20">
                                                                <label class="n_tx"><input type="checkbox" name="CheckboxGroup1" value="复选框" id="Checkbox"><i>瘦</i></label>
                                                                <label class="n_tx"><input type="checkbox" name="CheckboxGroup1" value="复选框" id="Checkbox1"><i>標準</i></label>
                                                                <label class="n_tx"><input type="checkbox" name="CheckboxGroup1" value="复选框" id="Checkbox2"><i>微胖</i></label>
                                                                <label class="n_tx"><input type="checkbox" name="CheckboxGroup1" value="复选框" id="Checkbox3"><i>胖</i></label>
                                                        </span>
                                </dt>
                                <dt class="matopj15">
                                                        <span>CUP</span>
                                                        <span class="line20">
                                                                <label class="n_tx"><input type="checkbox" name="CheckboxGroup1" value="复选框" id="Check"><i>A</i></label>
                                                                <label class="n_tx"><input type="checkbox" name="CheckboxGroup1" value="复选框" id="Check1"><i>B</i></label>
                                                                <label class="n_tx"><input type="checkbox" name="CheckboxGroup1" value="复选框" id="Check2"><i>C</i></label>
                                                                <label class="n_tx"><input type="checkbox" name="CheckboxGroup1" value="复选框" id="Check3"><i>D</i></label>
                                                                <label class="n_tx"><input type="checkbox" name="CheckboxGroup1" value="复选框" id="Check4"><i>E</i></label>
                                                                <label class="n_tx"><input type="checkbox" name="CheckboxGroup1" value="复选框" id="Check5"><i>F</i></label>
                                                        </span>
                                </dt>
                                                     <dt class="matopj15">
                                                         <div class="n_se left">
                                                             <span>婚姻</span>
                                                           <select name="" class="select_xx01"><option>未婚</option><option>已婚</option></select>
                                                         </div>
                                                         <div class="n_se right">
                                                             <span>喝酒</span>
                                                           <select name="" class="select_xx01"><option>是</option><option>否</option></select>
                                                         </div>
                                </dt>
                                <dt>
                                                       <span>搜索排列顺序</span>
                                                       <span><select name="" class="select_xx01"><option>登入時間</option><option>2019</option></select></span>
                                                  </dt>
                                   </div>
                                             <div class="n_txbut"><a  href="" class="n_dlbut ">搜索</a><a href="" class="n_zcbut">取消</a></div>
                                    
                </div>                    
                    
                               <div class="n_searchtit"><div class="n_seline"><span>搜索结果</span></div></div>
                               <div class="n_sepeop">
                                      <li class="nt_fg">
                                           <div class="n_seicon">
                                               <img src="/img/mem/b_01.png">
                                               <img src="/img/mem/b_02.png">
                                               <img src="/img/mem/b_03.png">
                                               <img src="/img/mem/b_04.png">
                                               <img src="/img/mem/b_05.png">
                                           </div>
                                           <div class="nt_photo"><img src="/img/mem/icon_03.png"></div> 
                                           <div class="nt_bot nt_bgco">
                                                  <h2>小周傑倫<span>25歲</span></h2>
                                                  <h3>臺北市<span>臺中區</span><span>職業<img src="/img/mem/icon_35.png" class="nt_img"></span></h3>
                                                  <h3>最後上線時間</h3>
                                           </div>         
                                      </li>
                                      <li class="nt_fg02">
                                           <div class="n_seicon">
                                               <img src="/img/mem/b_01.png">
                                               <img src="/img/mem/b_02.png">
                                               <img src="/img/mem/b_03.png">
                                               <img src="/img/mem/b_04.png">
                                               <img src="/img/mem/b_05.png">
                                           </div>
                                           <div class="nt_photo"><img src="/img/mem/icon_03.png"></div> 
                                           <div class="nt_bot nt_bgco02">
                                                  <h2>小周傑倫<span>25歲</span></h2>
                                                  <h3>臺北市<span>臺中區</span><span>職業<img src="/img/mem/icon_35.png" class="nt_img"></span></h3>
                                                  <h3>最後上線時間</h3>
                                           </div>         
                                      </li>
                                      <li class="nt_fg03">
                                           <div class="n_seicon">
                                               <img src="/img/mem/b_01.png">
                                               <img src="/img/mem/b_02.png">
                                               <img src="/img/mem/b_03.png">
                                               <img src="/img/mem/b_04.png">
                                               <img src="/img/mem/b_05.png">
                                           </div>
                                           <div class="nt_photo"><img src="/img/mem/icon_03.png"></div> 
                                           <div class="nt_bot nt_bgco03">
                                                  <h2>小周傑倫<span>25歲</span></h2>
                                                  <h3>臺北市<span>臺中區</span><span>職業</span></h3>
                                                  <h3>最後上線時間</h3>
                                           </div>         
                                      </li>
                                      <li class="nt_fg04">
                                           <div class="n_seicon">
                                               <img src="/img/mem/b_01.png">
                                               <img src="/img/mem/b_02.png">
                                               <img src="/img/mem/b_03.png">
                                               <img src="/img/mem/b_04.png">
                                               <img src="/img/mem/b_05.png">
                                           </div>
                                           <div class="nt_photo"><img src="/img/mem/icon_03.png"></div> 
                                           <div class="nt_bot nt_bgco04">
                                                  <h2>小周傑倫<span>25歲</span></h2>
                                                  <h3>臺北市<span>臺中區</span><span>職業</span></h3>
                                                  <h3>最後上線時間</h3>
                                           </div>         
                                      </li>
                                      <li class="nt_fg">
                                           <div class="n_seicon">
                                               <img src="/img/mem/b_01.png">
                                               <img src="/img/mem/b_02.png">
                                               <img src="/img/mem/b_03.png">
                                               <img src="/img/mem/b_04.png">
                                               <img src="/img/mem/b_05.png">
                                           </div>
                                           <div class="nt_photo"><img src="/img/mem/icon_03.png"></div> 
                                           <div class="nt_bot nt_bgco">
                                                  <h2>小周傑倫<span>25歲</span></h2>
                                                  <h3>臺北市<span>臺中區</span><span>職業</span></h3>
                                                  <h3>最後上線時間</h3>
                                           </div>         
                                      </li>
                                      <li class="nt_fg">
                                           <div class="n_seicon">
                                               <img src="/img/mem/b_01.png">
                                               <img src="/img/mem/b_02.png">
                                               <img src="/img/mem/b_03.png">
                                               <img src="/img/mem/b_04.png">
                                               <img src="/img/mem/b_05.png">
                                           </div>
                                           <div class="nt_photo"><img src="/img/mem/icon_03.png"></div> 
                                           <div class="nt_bot nt_bgco">
                                                  <h2>小周傑倫<span>25歲</span></h2>
                                                  <h3>臺北市<span>臺中區</span><span>職業</span></h3>
                                                  <h3>最後上線時間</h3>
                                           </div>         
                                      </li>
                                      
                               </div>
                                 <div class="fenye mabot30">
                                    <a href="">上一頁</a>
                                    <a href="">下一頁</a>
                                </div>
        </div>

      </div>
    </div>

    <div class="bot">
      <a href="">站長開講</a> 丨
      <a href=""> 網站使用</a> 丨
      <a href=""> 使用條款</a> 丨
      <a href=""> 聯絡我們</a>
      <img src="/img/mem/bot_10.png">
    </div>

  </body>

</html>