...
<h1>FunPoint VIP 付費取消資料</h1>
<form method="POST" action="@if (Auth::user()->can('admin')) {{ route('users/VIP/FunPointCancellations') }} @elseif (Auth::user()->can('readonly')) {{ route('users/VIP/FunPointCancellations/readOnly') }} @endif" class="search_form">
	{!! csrf_field() !!}
