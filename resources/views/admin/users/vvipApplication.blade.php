                            ...
                            <form id="form_{{$row->id}}" action="{{ route('users/VVIP_edit') }}" method="post">
                                ...
                                <div class="form-group supplement_notice_{{$row->id}}" @if($row->status!=3) style="display: none;" @endif>
                                    ...
                                    <textarea class="form-control" rows="3" name="supplement_notice">
                                        @if($row->supplement_notice ?? false){{$row->supplement_notice}}@else{{$row->name}} 您好，您於 {{$row->created_at->format("Y-m-d H:i")}} 申請本站VVIP，經站方審核，補件期限為 {{Carbon\Carbon::now()->addDays(7)->format("Y-m-d")." 23:59:59"}} 。@endif
                                    </textarea>
