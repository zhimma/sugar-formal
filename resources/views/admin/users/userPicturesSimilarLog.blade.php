...
@section('app-content')
    <style>
        ...
        div.block_line:last-child {
            float: left;
            margin-bottom: 1rem;
        }

        .images_comparation_cell b {
            clear: both;
            display: block;
        }

        #user_all_mem_pic_list,
        #user_all_avatar_list {
            width: 100% !important;
        }

        #user_all_mem_pic_list th,
        #user_all_avatar_list th {
            white-space: nowrap;
        }

        td.images_comparation_cell div img {
            border: 1px solid #ccc;
        }

        td.images_comparation_cell div.block_line,
        td.images_comparation_cell div.block_line img {
            max-width: 120px;
            margin-right: 10px;
        }
        ...
        @if (Auth::user()->can('admin') || Auth::user()->can('juniorAdmin'))
            ...
            <form action="/admin/users/picturesSimilarLog" method="get">
                <table class="table-hover table table-bordered" style="width: 50%;">
                    <tr>
                        <th>開始時間</th>
                        <td><input type='text' id="datepicker_1" name="date_start" data-date-format='yyyy-mm-dd'
                                value="@if (isset($_GET['date_start'])) {{ $_GET['date_start'] }}@else{{ old('date_start') }} @endif"
                                class="form-control"></td>
                    <tr>
                        <th>結束時間</th>
                        <td><input type='text' id="datepicker_2" name="date_end" data-date-format='yyyy-mm-dd'
                    ...
                    <tr>
                        <th>地區</th>
                        <td class="twzipcode">
                            <div class="twzip" id="city" data-role="county" data-name="city"
                                data-value="@if (isset($_GET['city'])) {{ $_GET['city'] }} @endif"></div>
                            <div class="twzip" id="area" data-role="district" data-name="area"
                                data-value="@if (isset($_GET['area'])) {{ $_GET['area'] }} @endif"></div>
                        </td>
                    </tr>
                    <tr>
                        <th>排序方式</th>
                        <td>
                            <div class="form-check form-check-inline">
                                <input type="radio" class="form-check-input" name="order_by" value="updated_at"
                                    @if (isset($_GET['order_by']) && $_GET['order_by'] == 'updated_at') checked @endif style="margin-left: unset;">
                                <label class="form-check-label" for="inlineRadio4">更新時間</label>
                            </div>
                            <div class="form-check form-check-inline">
                                <input type="radio" class="form-check-input" name="order_by" value="last_login"
                                    @if (isset($_GET['order_by']) && $_GET['order_by'] == 'last_login') checked @endif style="margin-left: unset;">
                                    ...
                                    <tr>
                                        @if ($loop->first)
                                            <td class="align-middle"
                                                style="{{ $ActionLog->target_user->isBanned($ActionLog->target_user->id) ? 'background-color: #FFFF00;' : '' }}"
                                                rowspan="{{ $count }}">
                                                <p>
                                                    @if ($ActionLog->target_user->meta->pic)
                                                        <img class="w-100"
                                                            src="{{ url($ActionLog->target_user->meta->pic) }}">
                                                    @else
                                                ...
                                                <p>
                                                    <span>會員名稱: <a
                                                            href="{{ route('users/advInfo', $ActionLog->target_user->id) }}"
                                                            target="_blank"><span
                                                                class="{{ $ActionLog->target_user->engroup == 2 ? 'text-danger' : 'text-primary' }}">{{ $ActionLog->target_user->name }}</span></a></span><br>
                                                ...
                                                <p>
                                                    @if ($ActionLog->target_user->isBanned($ActionLog->target_user->id))
                                                        <button class="btn btn-sm btn-success" type="button"
                                                            onclick="unblockModal(this)" data-toggle="modal"
                                                            data-target="#unblockModal"
                                                            data-uid="{{ $ActionLog->target_user->id }}"
                                                            data-reason="{{ $ActionLog->target_user->banned->reason }}"
                                                            data-expire="{{ $ActionLog->target_user->banned->expire_date }}">解除封鎖</button>
                                                    @else
                                                        <button class="btn btn-sm btn-danger" type="button"
                                                            onclick="blockModal(this)" data-toggle="modal"
                                                            data-target="#blockModal"
                                                            data-uid="{{ $ActionLog->target_user->id }}">封鎖</button>
                                                    @endif
                                                </p>
                                                <p>
                                                <form class="form-inline"
                                                    action="/admin/users/picturesSimilar/suspicious:toggle" method="post">
                                                    {!! csrf_field() !!}
                                                    <input type="hidden" name="uid"
                                                        value="{{ $ActionLog->target_user->id }}">
                                                    ...
                                                    @if ($ActionLog->target_user->suspicious)
                                                        ...
                                                        <input
                                                            class="form-control form-control-sm form-control-plaintext mr-sm-2"
                                                            type="text"
                                                            value="{{ $ActionLog->target_user->suspicious->reason }}"
                                                            readonly>
                                                        <button class="btn btn-sm btn-primary btn_sid" type="button"
                                                            data-confirm="0">解除</button>
                                                    @else
                                                        <input type="hidden" name="toggle" value="1">
                                                        <input class="form-control form-control-sm mr-sm-2" type="text"
                                                            placeholder="請輸入可疑原因" name="reason" value="">
                                                        <button class="btn btn-sm btn-danger btn_sid" type="button"
                                                            data-confirm="1">列入</button>
                                                    @endif
                                                </form>
                                                </p>
                                            </td>
                                                        ...
                                                        @if ($ImgResult->fullMatchingImages)
                                                            <p>
                                                                @foreach (json_decode($ImgResult->fullMatchingImages) as $fullMatchingImage)
                                                                    <a href="{{ $fullMatchingImage->url }}"
                                                                        target="_blank"><img
                                                                            src="{{ $fullMatchingImage->url }}"
                                                                            style="max-width:120px; margin-right:10px;"
                                                                            onerror="this.src='/img/linktosource.png'"></a>
                                                                @endforeach
                                                            </p>
                                                        @else
                                                            <p>沒有完全匹配的內容</p>
                                                        ...
                                                        @if ($ImgResult->partialMatchingImages)
                                                            <p>
                                                                @foreach (json_decode($ImgResult->partialMatchingImages) as $partialMatchingImage)
                                                                    <a href="{{ $partialMatchingImage->url }}"
                                                                        target="_blank"><img
                                                                            src="{{ $partialMatchingImage->url }}"
                                                                            style="max-width:120px; margin-right:10px;"
                                                                            onerror="this.src='/img/linktosource.png'"></a>
                                                                @endforeach
                                                            </p>
                                                        ...
                                                        @if ($ImgResult->pagesWithMatchingImages)
                                                            <p>
                                                                @foreach (json_decode($ImgResult->pagesWithMatchingImages) as $pagesWithMatchingImage)
                                                                    <a href="{{ $pagesWithMatchingImage->url }}"
                                                                        target="_blank">{{ $pagesWithMatchingImage->pageTitle ?? '無標題' }}</a><br>
                                                                @endforeach
                                                            </p>
                                                        @else
                                                            <p>沒有匹配的網頁</p>
                                                        ...
                                                        @if ($ImgResult->visuallySimilarImages)
                                                            <p>
                                                                @foreach (json_decode($ImgResult->visuallySimilarImages) as $visuallySimilarImage)
                                                                    <a href="{{ $visuallySimilarImage->url }}"
                                                                        target="_blank"><img
                                                                            src="{{ $visuallySimilarImage->url }}"
                                                                            style="max-width:120px; margin-right:10px;"
                                                                            onerror="this.src='/img/linktosource.png'"></a>
                                                                @endforeach
                                                            </p>
                                                            ...
                                                            @foreach ($sameImages as $sameImg)
                                                                @if ($sameImg->user)
                                                                    <div class="block_line">
                                                                        <a href="{{ $sameImg->pic }}" target="_blank"><img
                                                                                src="{{ $sameImg->pic }}"
                                                                                onerror="this.src='/img/linktosource.png'"></a>
                                                                        <div
                                                                            class="gender{{ $sameImg->user->engroup }} {{ $sameImg->userStateStr ? 'rs_user_' : '' }}{{ $sameImg->userStateStr ?? '' }}">
                                                                            <a href="{{ route('users/advInfo', $sameImg->user->id) }}"
                                                                                target="_blank">{{ $sameImg->user->name }}</a><span
                                                                                class="lli_holder">({{ $sameImg->userLliDiffDays }})</span>
                                                                        </div>
                                                                    </div>
                                                                @endif
                                                            @endforeach
                                                    ...
                                                    @if ($compareStatus ?? null)
                                                        ...
                                                        <div>
                                                            @forelse($compareRsImgs as $rsImg)
                                                                @if ($rsImg->user)
                                                                    <div class="block_line">
                                                                        <a href="{{ $rsImg->pic }}" target="_blank"><img
                                                                                src="{{ $rsImg->pic }}"
                                                                                onerror="this.src='/img/linktosource.png'"></a>
                                                                        <div
                                                                            class="gender{{ $rsImg->user->engroup }} {{ $rsImg->userStateStr ? 'rs_user_' : '' }}{{ $rsImg->userStateStr ?? '' }}">
                                                                            <a href="{{ route('users/advInfo', $rsImg->user->id) }}"
                                                                                target="_blank">{{ $rsImg->user->name }}</a><span
                                                                                class="lli_holder">({{ $rsImg->userLliDiffDays }})</span>
                                                                        </div>
                                                                    </div>
                                                                @endif
                                                            @empty
                    ...
                    <div class="modal-body">
                        <form id="BlockUser" action="/admin/users/picturesSimilar/block:toggle" method="post">
                            ...
                            <div class="form-group">
                                ...
                                <div>
                                    <button class="btn btn-sm btn-success text-white mb-1 mr-1 banReason"
                                        type="button">廣告</button>
                                    <button class="btn btn-sm btn-success text-white mb-1 mr-1 banReason"
                                        type="button">非徵求包養行為</button>
                                    <button class="btn btn-sm btn-success text-white mb-1 mr-1 banReason"
                                        type="button">用詞不當</button>
                                    <button class="btn btn-sm btn-success text-white mb-1 mr-1 banReason"
                                        type="button">照片不當</button>
                                    <button class="btn btn-sm btn-success text-white mb-1 mr-1 banReason"
                                        type="button">多重帳號</button>
                                    <button class="btn btn-sm btn-success text-white mb-1 mr-1 banReason"
                                        type="button">性別錯誤</button>
                                    <button class="btn btn-sm btn-success text-white mb-1 mr-1 banReason"
                                        type="button">要求裸照</button>
                                    <button class="btn btn-sm btn-success text-white mb-1 mr-1 banReason"
                                        type="button">罐頭訊息+主動給line</button>
                                    <button class="btn btn-sm btn-success text-white mb-1 mr-1 banReason"
                                        type="button">基本資料錯誤</button>
                                    <button class="btn btn-sm btn-success text-white mb-1 mr-1 banReason"
                                        type="button">站規8</button>
                                    <button class="btn btn-sm btn-success text-white mb-1 mr-1 banReason"
                                        type="button">未成年</button>
                                    <button class="btn btn-sm btn-success text-white mb-1 mr-1 banReason"
                                        type="button">拒往</button>
                                    <button class="btn btn-sm btn-success text-white mb-1 mr-1 banReason"
                                        type="button">多重帳號,情節嚴重</button>
                                    <button class="btn btn-sm btn-success text-white mb-1 mr-1 banReason"
                                        type="button">未依要求與站方聯絡，已按使用天數刷退VIP費用。並停止帳號使用。</button>
                                </div>
                            </div>
                            <div class="form-group">
