                ...
                <form action="/admin/users/toggleUserBlock" method="POST" id="clickToggleUserBlock">
                    ...
                    <div class="modal-body">
                        ...
                        <label style="margin:10px 0px;">
                            <input type="checkbox" name="addreason"
                        ...
                        <input placeholder="1.請輸入封鎖關鍵字" onfocus="this.placeholder=''"
                            onblur="this.placeholder='1.請輸入封鎖關鍵字'" class="form-control" type="text"
                            name="addautoban[]" rows="1">
                        <input placeholder="2.請輸入封鎖關鍵字" onfocus="this.placeholder=''"
                            onblur="this.placeholder='2.請輸入封鎖關鍵字'" class="form-control" type="text"
                            name="addautoban[]" rows="1">
                        <input placeholder="3.請輸入封鎖關鍵字" onfocus="this.placeholder=''"
                            onblur="this.placeholder='3.請輸入封鎖關鍵字'" class="form-control" type="text"
                            name="addautoban[]" rows="1">
                    </div>
    ...
    <script>
            ...
            $(".img_select").on("click", function() {
                ...
                for (var j = 0, c = imgBox.length; j < c; j++) {
                    $(".big_img .swiper-wrapper").append(
                        '<div class="swiper-slide"><div class="cell"><img src="' + imgBox.eq(j).find(
                            "img").attr("src") + '" / ></div></div>');
                }
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    response.banReason.forEach(function(value) {
                        $('#banReason').append('<a class="text-white btn btn-success banReason">' +
                            value.content + '</a>');
                    });
                    ...
                    response.cfp_id.forEach(function(value) {
                        $('#cfpid').append('<option value=' + value.cfp_id + '>' + value.cfp_id +
                            '</option>');
                    });
                    ...
                    response.userLogin_log.forEach(function(value) {
                        htmlstr = '';
                        htmlstr = htmlstr + '<tr class="loginItem" id="showloginTimeIP' + value
                            .loginMonth + '" data-sectionName="loginTimeIP' + value.loginMonth + '">';
                        htmlstr = htmlstr + '<td>';
                        htmlstr = htmlstr + '<span>' + value.loginMonth + ' [' + value.Ip.Ip_group
                            .length + ']' + '</span>';
                        htmlstr = htmlstr + '</td>';
                        htmlstr = htmlstr + '</tr>';
                        ...
                        for (let i = 0; i < value.Ip.Ip_group.length; i++) {
                            htmlstr = htmlstr + '<option value="' + value.Ip.Ip_group[i].ip + '">' +
                                '[' + value.Ip.Ip_group[i].loginTime + ']  ' + value.Ip.Ip_group[i].ip +
                                '</option>';
                        }
        ...
        function pic_tpl(picture) {
            ...
            if (picture.pic ?? false) {
                ...
                html_str = html_str + '<input type="checkbox" id="' + picture.pic.replace('/', '', ) +
                    '" name="pic[]" value="' + picture.pic + '" />';
                html_str = html_str + '<label for="' + picture.pic.replace('/', '', ) + '">';
                html_str = html_str + '<span>';

                if ((picture.operator ?? false) || ((picture.deleted_at ?? false) && picture.deleted_at !=
                        '0000-00-00 00:00:00')) {
