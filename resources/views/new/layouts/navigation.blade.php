...
{{--							@if($user->meta_()->isConsign == 0 && ($user->meta_()->consign_expiry_date == null||$user->meta_()->consign_expiry_date <= \Carbon\Carbon::now()))--}}
							<span class="getNum">
								...
								<span id="unreadCount">{{ $unread }}</span>
							</span>
{{--							@endif--}}
		...
		{{--									@if($user->meta_()->isConsign == 0 && ($user->meta_()->consign_expiry_date == null||$user->meta_()->consign_expiry_date <= \Carbon\Carbon::now()))--}}
											...
											<li>
												<a href="{!! url('dashboard/chat2/'.csrf_token().\Carbon\Carbon::now()->timestamp) !!}"><img src="/new/images/icon_45.png">收件夾</a><span id="unreadCount2">{{ $unread ?? 0 }}</span>
											</li>
											@if($user->engroup == 1)
												@php
