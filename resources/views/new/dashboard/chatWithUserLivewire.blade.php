...
<div class="bl bl_tab tab_payAlert" id="tab_payAlert">
    ...
    <a id="" onclick="gmBtnNoReload()" class="bl_gb bl_gb_fixed"><img src="/new/images/gb_icon.png"></a>
    <div class="n_blnr01 matop20 tip_adjust">
        ...
        <div class="n_bbutton">
            ...
            <span><a onclick="gmBtnNoReload()" class="n_right" href="javascript:">取消</a></span>
        </div>
    </div>
...
@section('javascript')
    <script>
        ...
        $(".GoDown").css('width',$('.message').width());
        $('.se_text_bot').css('width',$('.message').width());
        ...
        if(window.matchMedia("(min-width: 390px)").matches && window.matchMedia("(max-width: 767px)").matches){
            $('.bot').hide(); //footer
            bl_gb_fixed_top = $(window).height() / 5 + 10;
            $('.bl_gb_fixed').css('top',bl_gb_fixed_top);
            $('.bltitle_fixed').css('width',$('.tab_payAlert').width()+1);
        ...
        $(document).ready(function () {
            if($('.message').scrollTop()==($('.message').prop("scrollHeight") - $('.message').height())){
            ...
            $(".GoDown").click(function() {
                $(".message").animate({ scrollTop: ($(".message").prop("scrollHeight") - $(".message").height()) }, "slow", function() {
                    $(".GoDown").fadeOut( "slow", function() {
                        $(".GoDown").hide();
                    });
                });
        ...
        Echo.private('ChatUnsend.{{ $to->id }}.{{ auth()->user()->id }}')
            .listen('ChatUnsend', (e) => {
                if(!e.message.error) {
                    ...
                    window.livewire.emit('resetShowSlide');
                }
                else {
    ...
    $(document).ready(function () {
        ...
        objDiv.onscroll = function (ev) {
            ...
            if(objDiv.scrollTop == 0 && checkMoreData==1)
            {
                ...
                window.livewire.emit('resetIsLoad');
            }
        };
