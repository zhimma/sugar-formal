        ...
        <div class="row">
            ...
            <div class="col-sm-12 col-xs-12 col-md-10">
                <div class="shou"><span class="zq_font1">提報可疑對象</span>
                    <font class="zq_font2">Submission</font>
                    <a href="/dashboard/suspicious_list?s=false" class="toug_back btn_img">
                        ...
                        <form action="/dashboard/suspicious_doPosts?{{ csrf_token() }}={{ \Carbon\Carbon::now()->timestamp }}" id="posts" method="POST"  enctype="multipart/form-data">
                            ...
                            <div class="zap_ullist">
                                <div class="zap_ullist_a">
                                    ...
                                    <input type="hidden" name="suspicious_id" value="{{ $suspicious->id }}">
                                    <div class="xinzeng_a">
                                        <h2>提報類型
                                            <span class="xzinput">
                                             <select  name="type" class="x_input_a sec1">
                                                 <option value="">請選擇</option>
                                                 <option value="1" @if($suspicious->report_type==1) selected @endif>使用者糾紛(有見過面)</option>
                                                 <option value="2" @if($suspicious->report_type==2) selected @endif>車馬費詐騙(沒見過面)</option>
                                             </select>
                                         </span>
                                        </h2>
                                        <h2>糾紛對象
                                            <span class="xzinput">
                                             <select name="target_user_id" class="x_input_a sec1">
                                                 ...
                                                 @foreach($message_user_list as $user_id)
                                                     ...
                                                     @endphp
                                                     <option value="{{ $user_id }}" @if($suspicious->user_id==$user_id) selected @endif>{{ $user? $user->name : '' }}</option>
                                                 @endforeach
                                             </select>
                                         </span>
                                        </h2>
                                        <h2>銀行帳號<span class="xzinput"><input name="account_text" class="xte_input_a" placeholder="請輸入" value="{{ $suspicious->account_text }}"></span></h2>
                                        <h2>舉報原因<span class="xzinput"><textarea name="reason" placeholder="請平鋪直敘事發經過，請勿人身攻擊以及情緒用詞![本欄位最大字數300字]" class="xte_text">{{ $suspicious->reason }}</textarea></span>
                                        </h2>
                                    </div>
